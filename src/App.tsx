...
const AssignmentCard = ({ assignment, onSelect }: { assignment: any, onSelect: (a: any) => void }) => {
  const difficultyColors: { [key: string]: { bg: string, gradient: string } } = {
    Easy: { bg: '#10b981', gradient: 'linear-gradient(135deg, #10b981 0%, #059669 50%, #34d399 100%)' },
    Medium: { bg: '#f59e0b', gradient: 'linear-gradient(135deg, #f59e0b 0%, #d97706 50%, #fbbf24 100%)' },
    Hard: { bg: '#ef4444', gradient: 'linear-gradient(135deg, #ef4444 0%, #dc2626 50%, #f87171 100%)' }
  };

  const color = difficultyColors[assignment.difficulty] || difficultyColors.Easy;
...
const CipherSQLStudio = () => {
  ...
  const handleExecuteQuery = () => {
    ...
    // Add to history
    if (sqlQuery.trim() && (!queryHistory.length || queryHistory[queryHistory.length - 1] !== sqlQuery.trim())) {
      setQueryHistory(prev => [...prev, sqlQuery.trim()].slice(-10)); // Keep last 10
    }
    setHistoryIndex(-1);
    ...
    setTimeout(() => {
      ...
      if (result.success) {
        ...
        if (resultsSection) {
          resultsSection.classList.add('results-success');
          setTimeout(() => resultsSection.classList.remove('results-success'), 1000);
        }
      } else {
        setQueryResults(null);
    ...
    const handleKeyPress = (e: KeyboardEvent) => {
      ...
      // Arrow up/down for query history
        ...
        if (queryHistory.length > 0) {
          ...
          setSqlQuery(queryHistory[newIndex]);
        }
      }
      ...
      // H for hint
      else if (e.key === 'h' && !e.ctrlKey && !e.metaKey && document.activeElement?.tagName !== 'INPUT') {
        e.preventDefault();
        handleGetHint();
      }
    };
  ...
  }, [sqlQuery, view, isLoading, queryHistory, historyIndex]);
